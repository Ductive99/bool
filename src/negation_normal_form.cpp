...
/*   Updated: 2026/06/30 12:15:47 by esouhail         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::unique_ptr<ASTNode> transform_to_nnf(const ASTNode *node) {
	...
	if (node->type == NodeType::Xor) {
		auto new_node = ASTNode::make_binary(
			NodeType::And,
			ASTNode::make_binary(NodeType::Or, node->left->clone(),
								 node->right->clone()),
			ASTNode::make_binary(
				NodeType::Or,
				ASTNode::make_unary(NodeType::Not, node->left->clone()),
	...
	return ASTNode::make_binary(node->type, transform_to_nnf(node->left.get()),
